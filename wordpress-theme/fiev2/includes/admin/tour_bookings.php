...
if ( !defined('EXPEDITION_TOUR_BOOKINGS') ){
    define( "EXPEDITION_TOUR_BOOKINGS", "expedition_tour_bookings");
}

if ( !defined('EXPEDITION_TOUR_BOOKINGS_TITLE') ){
    define( "EXPEDITION_TOUR_BOOKINGS_TITLE", __('Registros', 'expedition'));
}
 ...
 * 
 */
class ExpeditionAdminTourBookingsTable extends WP_List_Table {
    ...
    function __construct() {
        parent::__construct(array(
            'singular' => EXPEDITION_TOUR_BOOKINGS.'_at', //Singular label
            'plural' => EXPEDITION_TOUR_BOOKINGS.'_at', //plural label, also this well be one of the table css class
    ...
    function get_columns() {
        $columns = array(
            ...
            'col_user_id' => __('Usuario','expedition'),
            // 'col_owner_id' => __('Tour Owner','expedition'),
            'col_tour_id' => __('Evento','expedition'),
            'col_meta' => __('Metadata','expedition'),
            'col_expeditioners' => __('Info. básica','expedition'),
            'col_status' => __('Status','expedition'),
            'col_source' => __('Source','expedition'),
                ...
                foreach ($columns as $column_name => $column_display_name) {
                    ...
                    switch ($column_name) {
                        ...
                        case "col_meta":
                            ...
                            ob_start();
                                ...
                                if ( $tour_meta && is_array($tour_meta) && count($tour_meta)>0 ){
                                    $_categories = get_field('category', $rec->tour_id);
                                    $categories = array();
                                    ...
                                    <table class="wf-striped-table wf-fixed-table">
                                        <thead>
                                            <tr>
                                                <th><?= __('Category', 'expedition') ?></th>
                                                <th><?= __('Title', 'expedition') ?></th>
                                                <th><?= __('Price', 'expedition') ?></th>
                                                <th><?= __('Discount', 'expedition') ?></th>
                                    ...
                                    foreach ($tour_meta as $key => $meta) {
                                        ...
                                        <tr <?= $key % 2 == 0 ? 'class="odd"' : 'class="even"' ?>>
                                            <td><?=$categories[$meta->category]?></td>
                                            <td><?=$meta->title?></td>
                                            <td><?=$meta->price?></td>
                                            <td><?=$rec->total_discount?></td>
                                            <td><?=$meta->qty?></td>
                                            <td><?=$rec->total_charged?></td>
                                        </tr>
                                        <?php
                                    }
                                    ?>
                                    ...
                                    <p style="text-align:right; margin-top: 20px;">
                                        <strong>Total Q<?= number_format($rec->total_charged,2) ?></strong>
                                    </p>
                                    <?php
                                }else{
                        ...
                        case "col_expeditioners":
                            ...
                            ob_start();
                                ...
                                if ( $data && is_array($data) && count($data)>0 ){
                                    ?>
                                    <table class="wf-striped-table wf-fixed-table" style="width:900px;">
                                        <thead>
                                            <tr>
                                                ...
                                                <th><?= __('Teléfono', 'expedition') ?></th>
                                                
                                                
                                                <th><?= __("Edad", 'expedition') ?></th>
                                                <th><?= __("Sexo", 'expedition') ?></th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                    <?php
                        ...
                        case "col_status": echo '<td ' . $attributes . '>';
                                ...
                                if ( $rec->status == BOOKING_PENDING_CONFIRM || $rec->status == BOOKING_CONFIRMED_CARD){
                                ...
                                $nonce2 = wp_create_nonce("cancel_single_booking");
                                if ( $rec->status != BOOKING_CANCELLED ){
                                echo '<p class="buttons"><a href="#" '
                                    .'data-nonce="' . $nonce2 . '" '
                                    .'data-booking_id="' . $rec->id . '" '
                                    . 'class="cancel_single_booking button button-small">'.__('Cancel', 'expedition' ).'</a></p>';
                                }
...
if( current_user_can('edit_tours') ){
    new ExpeditionAdminTourBookingsTable_Settings();
}


class ExpeditionAdminTourBookingsTable_Settings {
    ...
    function panel_pages() {
        add_menu_page( EXPEDITION_TOUR_BOOKINGS_TITLE, EXPEDITION_TOUR_BOOKINGS_TITLE, 'edit_tours', EXPEDITION_TOUR_BOOKINGS, array(&$this, 'bootstrap'), 'dashicons-format-aside', 18 );
    ...
    public function bootstrap() {
        ...
        $admin_table = new ExpeditionAdminTourBookingsTable();
        ...
        <div class="wrap">
            ...
            <h2>
                <?=  __('Lista de parcipantes', 'expedition')  ?>
            </h2>
            ...
            <form method="get" class="expedition_search_form">
                <input type="hidden" name="page" value="<?=EXPEDITION_TOUR_BOOKINGS?>">
